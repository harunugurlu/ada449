...
const student_number::Int64 = 28925575182  ## <---replace 0 by your student_number 
### ---- ###
...
function objective(A::AbstractMatrix, B::AbstractMatrix)::AbstractFloat
    ...
    if size(A,1) != size(A,2) || det(A) == 0
        throw(ArgumentError("Matrix A should be square and have non-zero determinant."))
    end    
    AB = A * B
    Id = Matrix(1.0I, size(A))
    return norm(AB - Id)^2
end
...
for i in 1:max_iter
    ...
    ∇B = gradient(B -> objective(A, B), B)[1]

    B -= lr * ∇B
    
    if norm(∇B) < stopping_criterion
        break
    end
end
return B
